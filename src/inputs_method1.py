...
def inputs_1(directory):
    ...
    print()
    ## TRAIN SPLIT
    inp_train = input[:idx1980].reshape(-1,input.shape[1])
    inp_true_train = input[1:idx1980 + 1].reshape(-1,input.shape[1])
    dt_train = dt[:idx1980].reshape(-1,1)
    time_years_train = time_years[1:idx1980 + 1]
    var_train = np.var(inp_train)
    print("Train set size : %d and variance_train = %.3f"%(len(inp_train), var_train))
    print("Train vectors shapes :", inp_train.shape, inp_true_train.shape, dt_train.shape)
    print()
    ...
    print("Test  vectors shapes :", inp_test.shape, inp_true_test.shape, dt_test.shape)

    plt.figure(figsize = (20,4))
    plt.plot(time_years_train, inp_true_train[:,0], color = 'dodgerblue', label = 'Chl CPR,stand on P1 (train)')
    plt.plot(time_years_test, inp_true_test[:,0], color = 'deeppink', label = 'Chl CPR,stand on P2 (test)')
    plt.plot(time_years, np.zeros_like(time_years), color = 'dimgrey', linestyle = 'dashed', label = 'Mean P1')
    plt.plot(time_years_test, np.mean(inp_true_test[:,0])*np.ones_like(time_years_test), color = 'darkviolet', label = 'Mean P2')
    plt.title("Train and test sets.", size = 20, fontweight = 'bold')
